...
/*   Updated: 2026/01/31 18:46:16 by fpedroso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static char			**merge_env_lists_into_arr(t_var_lst *inline_lst, t_var_lst *persist_lst);
static t_var_lst	*merge_and_unduplicate(t_var_lst *inline_lst, t_var_lst *persist_lst);
...
char	**get_current_envs(t_env_vars env_lists)
{
	...
	current_envs = merge_env_lists_into_arr(env_lists.inline_envs, env_lists.persistent_envs_ptr);
	if (!current_envs)
	{
